...
data_base_dir = "/global/home/users/yifengh3/VAE/vec_data/recon_data/pfn_sampled"
raw_b_signals = np.load(os.path.join(data_base_dir, "pfn_bsignal_no_ht_sampled.npz")) 
raw_hv_signals = np.load(os.path.join(data_base_dir, "pfn_hv_vector_no_ht_sampled.npz")) 
...
for signal_1, signal_2 in zip(signal_1_recons,signal_2_recons):
    ...
    aucs.append(auc)
    np.savetxt('results/auc_log_v2.csv', np.array(aucs), delimiter=',') 
...
np.savez("results/auc_v3.npz", beta= log_betas, aucs=aucs)
